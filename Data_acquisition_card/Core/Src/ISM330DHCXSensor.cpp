...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_GetOutputDataRate(float *Odr) {
	...
	if (ism330dhcx_xl_data_rate_get(&reg_ctx, &odr_low_level)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_GetFullScale(int32_t *FullScale) {
	...
	/* Read actual full scale selection from sensor */
	if (ism330dhcx_xl_full_scale_get(&(reg_ctx), &fs_low_level)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::GYRO_GetOutputDataRate(float *Odr) {
	...
	if (ism330dhcx_gy_data_rate_get(&reg_ctx, &odr_low_level)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::GYRO_GetFullScale(
		int32_t *FullScale) {
	ISM330DHCXStatusTypeDef ret = ISM330DHCX_OK;
	ism330dhcx_fs_g_t fs_low_level;

	/* Read actual full scale */
	if (ism330dhcx_gy_full_scale_get(&reg_ctx, &fs_low_level)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_GetEventStatus(
...
//  (void)memset((void *)Status, 0x0, sizeof(ISM330DHCX_Event_Status_t));
	...
	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_WAKE_UP_SRC,
			(uint8_t*) &wake_up_src, 1) != ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_TAP_SRC, (uint8_t*) &tap_src,
			1) != ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_D6D_SRC, (uint8_t*) &d6d_src,
			1) != ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_MD1_CFG, (uint8_t*) &md1_cfg,
			1) != ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_MD2_CFG, (uint8_t*) &md2_cfg,
			1) != ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_INT1_CTRL,
			(uint8_t*) &int1_ctrl, 1) != ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Set_SelfTest(uint8_t Status) {
	...
	reg = (Status == 0U) ? ISM330DHCX_XL_ST_DISABLE :
			...
			(Status == 2U) ?
					ISM330DHCX_XL_ST_NEGATIVE : ISM330DHCX_XL_ST_DISABLE;
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Get_SelfTest(
		uint8_t *SelfStatus) {
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Enable_HP_Filter(
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::GYRO_Set_SelfTest(uint8_t Status) {
	...
	reg = (Status == 0U) ? ISM330DHCX_GY_ST_DISABLE :
			...
			(Status == 3U) ?
					ISM330DHCX_GY_ST_NEGATIVE : ISM330DHCX_GY_ST_DISABLE;
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::GYRO_Get_SelfTest(
		uint8_t *SelfStatus) {
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::GYRO_Get_DRDY_Status(
		uint8_t *Status) {
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::GYRO_Enable_HP_Filter(
		ism330dhcx_hpm_g_t CutOff) {
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_EnableFreeFallDetection(
	...
	/* FF_THS setting */
	if (ism330dhcx_ff_threshold_set(&reg_ctx, ISM330DHCX_FF_TSH_312mg)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_DisableFreeFallDetection() {
	...
	/* FF_THS setting */
	if (ism330dhcx_ff_threshold_set(&reg_ctx, ISM330DHCX_FF_TSH_156mg)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_SetFreeFallThreshold(
		uint8_t Threshold) {
	if (ism330dhcx_ff_threshold_set(&reg_ctx, (ism330dhcx_ff_ths_t) Threshold)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_SetFreeFallDuration(
		uint8_t Duration) {
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_EnableWakeUpDetection(
		ISM330DHCX_SensorIntPin_t IntPin) {
	ISM330DHCXStatusTypeDef ret = ISM330DHCX_OK;
	ism330dhcx_pin_int1_route_t val1;
	ism330dhcx_pin_int2_route_t val2;
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_SetWakeUpThreshold(
		uint8_t Threshold) {
	/* Set wake up threshold */
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_SetWakeUpDuration(
		uint8_t Duration) {
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_EnableSingleTapDetection(
		ISM330DHCX_SensorIntPin_t IntPin) {
	ISM330DHCXStatusTypeDef ret = ISM330DHCX_OK;
	ism330dhcx_pin_int1_route_t val1;
	ism330dhcx_pin_int2_route_t val2;
	...
	if (ism330dhcx_tap_detection_on_x_set(&reg_ctx, PROPERTY_ENABLE)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	/* Enable Y direction in tap recognition. */
	if (ism330dhcx_tap_detection_on_y_set(&reg_ctx, PROPERTY_ENABLE)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	/* Enable Z direction in tap recognition. */
	if (ism330dhcx_tap_detection_on_z_set(&reg_ctx, PROPERTY_ENABLE)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_DisableSingleTapDetection() {
	...
	if (ism330dhcx_tap_detection_on_z_set(&reg_ctx, PROPERTY_DISABLE)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	/* Disable Y direction in tap recognition. */
	if (ism330dhcx_tap_detection_on_y_set(&reg_ctx, PROPERTY_DISABLE)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}

	/* Disable X direction in tap recognition. */
	if (ism330dhcx_tap_detection_on_x_set(&reg_ctx, PROPERTY_DISABLE)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_EnableDoubleTapDetection(
		ISM330DHCX_SensorIntPin_t IntPin) {
	ISM330DHCXStatusTypeDef ret = ISM330DHCX_OK;
	ism330dhcx_pin_int1_route_t val1;
	ism330dhcx_pin_int2_route_t val2;
	...
	if (ism330dhcx_tap_mode_set(&reg_ctx, ISM330DHCX_BOTH_SINGLE_DOUBLE)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_DisableDoubleTapDetection() {
	...
	if (ism330dhcx_tap_mode_set(&reg_ctx, ISM330DHCX_ONLY_SINGLE)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_SetTapThreshold(
		uint8_t Threshold) {
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Enable6DOrientation(
		ISM330DHCX_SensorIntPin_t IntPin) {
	ISM330DHCXStatusTypeDef ret = ISM330DHCX_OK;
	ism330dhcx_pin_int1_route_t val1;
	ism330dhcx_pin_int2_route_t val2;
	...
	if (ism330dhcx_6d_threshold_set(&reg_ctx, ISM330DHCX_DEG_60)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Disable6DOrientation() {
	...
	if (ism330dhcx_6d_threshold_set(&reg_ctx, ISM330DHCX_DEG_80)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Set6DOrientationThreshold(
		uint8_t Threshold) {
	if (ism330dhcx_6d_threshold_set(&reg_ctx, (ism330dhcx_sixd_ths_t) Threshold)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Get6DOrientationXL(
		uint8_t *XLow) {
	ism330dhcx_d6d_src_t data;

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_D6D_SRC, (uint8_t*) &data, 1)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Get6DOrientationXH(
		uint8_t *XHigh) {
	ism330dhcx_d6d_src_t data;
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Get6DOrientationYL(
		uint8_t *YLow) {
	ism330dhcx_d6d_src_t data;
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Get6DOrientationYH(
		uint8_t *YHigh) {
	ism330dhcx_d6d_src_t data;
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Get6DOrientationZL(
		uint8_t *ZLow) {
	ism330dhcx_d6d_src_t data;
 ...
 *  @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Get6DOrientationZH(
		uint8_t *ZHigh) {
	ism330dhcx_d6d_src_t data;
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::FIFO_Get_Num_Samples(
		uint16_t *NumSamples) {
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::FIFO_Get_Full_Status(
		uint8_t *Status) {
	ism330dhcx_reg_t reg;

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_FIFO_STATUS2, &reg.byte, 1)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::FIFO_Set_INT1_FIFO_Full(
		uint8_t Status) {
	ism330dhcx_reg_t reg;

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_INT1_CTRL, &reg.byte, 1)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
	...
	if (ism330dhcx_write_reg(&reg_ctx, ISM330DHCX_INT1_CTRL, &reg.byte, 1)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::FIFO_Set_INT2_FIFO_Full(
		uint8_t Status) {
	ism330dhcx_reg_t reg;

	if (ism330dhcx_read_reg(&reg_ctx, ISM330DHCX_INT2_CTRL, &reg.byte, 1)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
	...
	if (ism330dhcx_write_reg(&reg_ctx, ISM330DHCX_INT2_CTRL, &reg.byte, 1)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::FIFO_Set_Watermark_Level(
		uint16_t Watermark) {
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::FIFO_ACC_Get_Axes(
 ...
 * @retval 0 in case of success, an error code otherwise
 */
ISM330DHCXStatusTypeDef ISM330DHCXSensor::FIFO_GYRO_Get_Axes(
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::ACC_Enable_DRDY_On_INT1() {
	...
	if (ism330dhcx_pin_int1_route_get(&reg_ctx, &pin_int1_route)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
	...
	if (ism330dhcx_pin_int1_route_set(&reg_ctx, &pin_int1_route)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::GYRO_Enable_DRDY_On_INT2() {
	...
	if (ism330dhcx_pin_int2_route_get(&reg_ctx, &pin_int2_route)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
	...
	if (ism330dhcx_pin_int2_route_set(&reg_ctx, &pin_int2_route)
			!= ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
...
ISM330DHCXStatusTypeDef ISM330DHCXSensor::DRDY_Set_Mode(uint8_t Mode) {
	...
	if (ism330dhcx_data_ready_mode_set(&reg_ctx,
			(ism330dhcx_dataready_pulsed_t) Mode) != ISM330DHCX_OK) {
		return ISM330DHCX_ERROR;
	}
