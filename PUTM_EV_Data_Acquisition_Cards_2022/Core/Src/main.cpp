...
static void MX_GPIO_Init(void)
{
  ...
  GPIO_InitStruct.Pin = kill_driver_input_Pin|kill_right_input_Pin|kill_left_input_Pin|inertia_input_Pin
                          |overtravel_input_Pin;
  ...
  /* USER CODE BEGIN 5 */
	...
	for (;;) {
		...
		can_main_frame_send(adc_data[Analog_channel::SUSP_R],
				...
				braking_pressure_percent_calculation(
						adc_data[Analog_channel::BRAKE_FRONT]),
				braking_pressure_percent_calculation(
						adc_data[Analog_channel::BRAKE_BACK]), status);
		can_acceleraton_frame_send((int16_t) acc[0], (int16_t) acc[1],(int16_t) acc[2]);
		can_gyroscope_frame_send((int16_t)gyr[0], (int16_t)gyr[1], (int16_t)gyr[2]);
	}
  /* USER CODE END 5 */
}
